...
def initialize_per_user_data():
    for e in (
        ...
        .query(
            ...
            func.sum(Transcript.data["payload"]["duration"].cast(Float)),
            Transcript.created_by,
        )
        .filter(Transcript.data["payload"]["skipped"].cast(Boolean) == False)
        .group_by(Transcript.created_by)
        .all()
    ):
        user = e[2]

        data = {"user": user, "segments": e[0], "duration": e[1]}
...
# Add the arguments
parser.add_argument("--audio-dir", type=str, required=True, help="Root directory for audio files.")
parser.add_argument("--transcripts-dir", type=str, required=True, help="Root directory for transcripts.")
